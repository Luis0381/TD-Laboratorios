/*  TRANSMISION DE DATOS 2023 - Laboratorio N°2
...
void ConfigurarMODEM(LPC_USART_T *pUART_USB, LPC_USART_T *pUART_MODEM){
    ...
    // el if es solamente para desactivarlo si ya lo hemos configurado, no le den mucha importancia

    if(1){
        // ENVIAR COMANDOS AT PARA CONFIGURAR

         // Verifico la comunicacion:
        EnviarComandoAT(pUART_MODEM, "AT\r\n");
        
        // Asigno un nombre al dispositivo:
        EnviarComandoAT(pUART_MODEM, "AT+NAME=AguanteElChef\r\n");
        
        // Asigno una password:
        EnviarComandoAT(pUART_MODEM, "AT+PSWD=30isthebest\r\n");

        // Configuro la comunicacion:
        EnviarComandoAT(pUART_MODEM, "AT+UART=38400,1,0\r\n");
        
        // Configuro el rol como esclavo:
        EnviarComandoAT(pUART_MODEM, "AT+ROLE=0\r\n");

        // Habilito perfil SPP:
        EnviarComandoAT(pUART_MODEM, "AT+INIT\r\n");

        // Inicio una busqueda de dispositivos Bluetooth cercanos:
   ...
   while (1) {
       ...
       if(UARTLeerByte(MODEM_UART, &readData, &readError)){
          if(readError){
              ...
              // Implemento la logica del contador
              if(readData == 'q'){
                  if (contador == 99)
                  {
                    contador=0;
